<template>
  <header
    class="relative z-10 border-b border-white/10 flex items-center justify-between px-6 h-16 shrink-0 bg-cover bg-left-top"
  ...
  >
    ...
    <div class="w-10 h-10 shrink-0">
      <button
        v-if="showBack"
        class="w-10 h-10 flex items-center justify-center hover:opacity-70 transition-opacity rounded-full"
        @click="goBack()"
        aria-label="返回"
      >
        <img :src="backArrow" alt="返回" class="w-6 h-6 object-contain" />
      </button>
    </div>
